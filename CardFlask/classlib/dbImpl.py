...
class dbImpl:
    ...
    def __init__(self):
        self.dname = "/Users/apple/CardGame/CardFlask/classlib/doudizhu.db"
    def createPlayer(self,player):
        conn = sqlite3.connect(self.dname)
        c= conn.cursor()
    ...
    def getPlayerById(self,id):
        conn = sqlite3.connect(self.dname)
        c= conn.cursor()
        c.execute("select * from player where player.id=?", (str(id)))
    ...
    def deletePlayerById(self,id):
        conn = sqlite3.connect(self.dname)
        c= conn.cursor()
        c.execute("delete from player where id=?", (str(id),))
        conn.commit()
        conn.close()
